...
func NewPluginStore(api plugin.API, logger bot.Logger, tracker tracker.Tracker, enableEncryption bool, encryptionKey []byte) Store {
	...
	return &pluginStore{
		...
		oauth2KV:           oauth2KV,
		welcomeIndexKV:     kvstore.NewCacheKeyStore(kvstore.NewHashedKeyStore(basicKV, WelcomeKeyPrefix), time.Second*10),
		settingsPanelKV:    kvstore.NewCacheKeyStore(kvstore.NewHashedKeyStore(basicKV, SettingsPanelPrefix), time.Second*10),
		Logger:             logger,
		Tracker:            tracker,
	}
}
